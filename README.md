...
- Load raw data from Le Wagon Big Query all at once in memory (no chunk)
...
Congratulation, you just grow from a **Data Scientist** into a full **ML Engineer**!
...
- `data.py` has refactored 2 methods that we'll use heavily in `main.py`
  - `get_data_with_cache()` (get some data from Big Query or cached CSV if it exists)
...
```bash
make reinstall_package # always check what make does in Makefile
```
...
Tips: The CLI can interpolate `.env` variables if you prefix them with a `$` sign (e.g. `$GCP_REGION`)
<details>
...
gsutil mb \                             # make bucket
    -l $GCP_REGION \                    #   (This is a multi-line command.
    -p $GCP_PROJECT \                   #    We use the backslashes to
    gs://$BUCKET_NAME                   #    continue on the next line.)
...
- It's optimized for large transformation such as `group by`, `join`, `where` etc.
- But it's not optimized for frequent row-by-row insertions/deletions

Le WagonCab is actually using a managed PostgreSQL Database Service (e.g. [Google Cloud SQL](https://cloud.google.com/sql)) as its main production database on which its Django app is storing / reading hundreds of thousands of individual transactions per day!

Every night, Le WagonCab launches a "database replication" job that applies the daily diffs of the "main" PostgresSQL database into the "replica" Big Query warehouse. Why?
...
👉 Run `make show_sources_all` to see that you're back to a blank state!
